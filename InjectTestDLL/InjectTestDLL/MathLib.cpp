...
void hUpdate(void* Spawner) //как входной параметр приходит указатель на Spawner
{
	...
	if (mySpawner)
	{
		PBYTE foo = reinterpret_cast<PBYTE>((moduleAssembly + 0x3D8F50)); //число 16 преобразуется в целочисленный указатель на начало метода spawnBonus
		reinterpret_cast<tspawnBonus>(foo)(mySpawner);
		//Для статических методов не надо знать указатель на mySpawner
...
void Hook(void* src, void* dst, unsigned int len)
{
	...
	//затирание пустыми инструкциями
	// заполнение памяти  на указанную длину  числом 0x90
	memset(src, 0x90, len);
	...
	//в ячейку начала функции записывают команду относительного прыжка
	*(BYTE*)src = 0xE9;
	//берут адрес со смещением на 1 инструкцию. По новому адресу записывают в него значение для прыжка
